...
export default function ProductPage({ selectedTab, tableData, setTableData, fetchTableData }) {
  ...
  const handlePageChange = (newPage) => {
    if (newPage > 0 && newPage <= Math.ceil(tableData.total / tableData.limit)) {
      fetchTableData(selectedTab, newPage, filterProduct.sizePerPage, null, null, null, filterProduct.keyword);
    }
  };

  const handleChangeSizePerPage = (sizePerPage) => {
    setFilterProduct((prevState) => ({...prevState, sizePerPage: sizePerPage}))
    fetchTableData(selectedTab, 1, sizePerPage, null, null, null, filterProduct.keyword);
  }
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          fetchTableData(selectedTab, 1, filterProduct.sizePerPage, null, null, null, filterProduct.keyword);
        }
      });
    } catch (error) {
      Swal.fire({
  ...
  const handlePrintReport = async (e) => {
          ...
          try {    
            const response = await fetch(`${baseUrlAPI}/print/products`, {
              method: "POST",
              headers: {
    ...
    <div className="w-full flex flex-col gap-10 mx-auto ease-in duration-150">
      <div className="flex flex-col md:flex-row justify-between">
        ...
        <div className="flex gap-4">
            ...
            <button onClick={() => setIsModalProductOpen({ action: "ADD", isOpen: true })} className="w-full hover:bg-orange-500 duration-100 ease-in max-w-fit font-bold text-2xl text-white text-center bg-orange-400 px-5 py-3 rounded-full shadow-md">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="size-6">
